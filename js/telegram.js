...
function uploadChunkXhr(blob,name,tid,chunkLabel,onProgress){
...
async function uploadOne(file){
  ...
  for(let i=0;i<totalChunks;i++){
    ...
    const result=await uploadChunkXhr(blob,cname,tid,`Chunk ${i+1}/${totalChunks}`,(loaded,total)=>{
      const cp=loaded/total;const overall=Math.round(((i+cp)/totalChunks)*100);
      setUI(file.name,`${isChunked?`Part ${i+1}/${totalChunks} · `:''}${(loaded/(1024*1024)).toFixed(1)}/${(total/(1024*1024)).toFixed(1)} MB`,overall);
      tmUpdate(tid,overall);
    });
...
async function downloadFile(f){
  ...
  try{
    const blobs=[];for(const c of f.chunks){const u=await tgFileUrl(c.fileId);if(!u)throw new Error('No URL');const r=await fetch(u);blobs.push(await r.blob());}
...
async function deleteFolder(folder){
  ...
  const toDelete=S.db.files.filter(f=>ids.includes(f.folderId));
...
async function openMedia(f){
  ...
  if(url){if(isImg){const img=document.createElement('img');img.className='media-img';img.src=url;let z=false;img.onclick=()=>{z=!z;img.style.transform=z?'scale(2.2)':'';};body.innerHTML='';body.appendChild(img);}else if(isVid){const v=document.createElement('video');v.className='media-vid';v.controls=v.autoplay=true;v.src=url;body.innerHTML='';body.appendChild(v);}else if(isAud){body.innerHTML=`<div style="text-align:center;padding:2rem"><i class="fas fa-music" style="font-size:4rem;color:var(--primary);display:block;margin-bottom:1.5rem"></i><audio controls style="width:80%;max-width:400px" src="${url}"></audio></div>`;}}
